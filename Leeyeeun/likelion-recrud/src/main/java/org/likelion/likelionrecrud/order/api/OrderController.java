...
public class OrderController {
    ...
    public ResponseEntity<OrderListResDto> getOrderByMember(@PathVariable("memberId") Long memberId) {
        List<OrderInfoResDto> orderInfiResDtoList = orderService.findOrderInfoByMemberId(memberId);
        return ResponseEntity.ok(OrderListResDto.from(orderInfiResDtoList));
    }

    @PostMapping("/{orderId}")
    public ResponseEntity<String> cancelOrder(@PathVariable("orderId")Long orderId) {
        orderService.cancelOrder(orderId);
        return ResponseEntity.ok("주물이 취소되었습니다 !");
    }

    @DeleteMapping("{orderId}")
    public ResponseEntity<String> deleteOrder(@PathVariable("orderId")Long orderId) {
        orderService.deleteOrder(orderId);
        return ResponseEntity.ok("주문의 삭제되었습니다 !");
    }
