package org.likelion.likelionassignmentrecrud.category.application;

import lombok.RequiredArgsConstructor;
import org.likelion.likelionassignmentrecrud.category.api.dto.request.CategorySaveReqDto;
import org.likelion.likelionassignmentrecrud.category.api.dto.request.CategoryUpdateReqDto;
import org.likelion.likelionassignmentrecrud.category.api.dto.response.CategoryInfoResDto;
import org.likelion.likelionassignmentrecrud.category.api.dto.response.CategoryListResDto;
import org.likelion.likelionassignmentrecrud.category.domain.Category;
import org.likelion.likelionassignmentrecrud.category.domain.repository.CategoryRepository;
...
public class CategoryService {
    ...
    public void categorySave(CategorySaveReqDto categorySaveReqDto) {
        Category parentCategory = null; // 부모 카테고리 객체
        if (categorySaveReqDto.parentId() != null) { // parentId가 null이 아니면
            parentCategory = categoryRepository.findById(categorySaveReqDto.parentId()) // parentCategory에 parentId 저장
                    .orElseThrow(() -> new IllegalArgumentException("잘못된 요청"));
        }
        Category category = Category.builder()
                .parent(parentCategory) // parentCategory에 저장해둔 parentId를 Category parentId에 저장
