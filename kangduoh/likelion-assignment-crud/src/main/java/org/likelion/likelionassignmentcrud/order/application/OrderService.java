...
import org.likelion.likelionassignmentcrud.order.api.dto.response.OrderItemResDto;
...
import org.likelion.likelionassignmentcrud.order.domain.OrderStatus;
import org.likelion.likelionassignmentcrud.order.domain.repository.OrderItemRepository;
...
public class OrderService {
    ...
    public void orderSave(OrderSaveReqDto orderSaveReqDto) {
        ...
        for (int i = 0; i < orderSaveReqDto.itemIds().size(); i++) {
            ...
            Item item = itemRepository.findById(itemId)
                    .orElseThrow(() -> new IllegalArgumentException("해당 상품 없습니다. id = " + itemId));
    ...
    public List<OrderInfoResDto> findOrderInfoByMemberId(Long memberId) {
        Member member = memberRepository.findById(memberId)
                .orElseThrow(() -> new IllegalArgumentException("해당 회원 없습니다. + id" + memberId));
